...
function ProgramStartPicker({ open, programs, onClose, onPickProgram, onStartBlank, onGoCreate }) {
  return html`
    <${Modal} open=${open} onClose=${onClose} title="Velg program">
      ${programs.length === 0 ? html`
        <div class="text-center py-6">
          <div class="text-5xl mb-3">🧱</div>
          <div class="font-medium mb-1">Ingen programmer</div>
          <div class="text-sm text-ink-500 mb-5">Lag et treningsprogram først, så kan du starte økter fra det.</div>
          <${Button} variant="primary" className="w-full" onClick=${() => { onClose(); onGoCreate(); }}>Lag et program<//>
          <button onClick=${() => { onClose(); onStartBlank(); }} class="tap mt-3 text-sm text-ink-500 active:text-ink-700">Start en økt uten program</button>
        </div>
      ` : html`
        <div class="space-y-3">
          ${programs.map(p => html`
            <div>
              <div class="text-sm font-semibold mb-1.5">${p.name}</div>
              <div class="space-y-1.5">
                ${p.days.map((day, i) => html`
                  <button
                    onClick=${() => { onClose(); onPickProgram(p, i); }}
                    class="tap w-full text-left px-4 py-3 rounded-xl bg-ink-200 text-ink-900 active:bg-ink-300 flex items-center justify-between"
                  >
                    <div>
                      <div class="font-medium">${day.name}</div>
                      <div class="text-xs text-ink-500 mt-0.5">${day.exercises.length} ${day.exercises.length === 1 ? 'øvelse' : 'øvelser'}</div>
                    </div>
          ...
          `)}
          <div class="pt-3 border-t border-ink-200 text-center">
            <button onClick=${() => { onClose(); onStartBlank(); }} class="tap text-sm text-ink-500 active:text-ink-700 py-2">Start en økt uten program</button>
          </div>
        </div>
      `}
    <//>
...
function HomeScreen({ data, update, navigate }) {
  ...
  // Når "Start økt" trykkes: åpne program-velgeren slik at brukeren
  // velger et program. Hvis ingen programmer finnes, viser velgeren
  // en empty-state med snarvei til å lage et.
  const openStartPicker = () => setProgramPickerOpen(true);
  ...
  return html`
    <div class="screen-enter pb-24">
      ...
      ${!data.activeWorkout && html`
        <div class="px-5 mb-5">
          <${Button} variant="primary" size="lg" className="w-full" onClick=${openStartPicker}>
      ...
      <${ProgramStartPicker}
        ...
        onPickProgram=${(p, i) => startFromProgram(p, i)}
        onStartBlank=${startBlankWorkout}
        onGoCreate=${() => navigate('programmer')}
      />


      <!-- Uke-oversikt — trykk for å åpne kalender -->
      <button
        class="tap block w-full text-left px-5 mb-6 group"
        onClick=${() => navigate('kalender')}
      >
        <div class="flex items-center justify-between mb-3">
          <div class="text-xs uppercase tracking-wide text-ink-400 font-medium flex items-center gap-1.5">
            Denne uka
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-ink-400 group-active:translate-x-0.5 transition-transform"><path d="m9 6 6 6-6 6"/></svg>
          </div>
...
function CalendarScreen({ data, navigate }) {
  const today = new Date();
  const [view, setView] = useState(() => ({
    year: today.getFullYear(),
    month: today.getMonth(), // 0-indeksert
  }));

  // Bygg map fra startOfDay-ts til array av økter den dagen
  const workoutsByDay = useMemo(() => {
    const map = new Map();
    for (const w of data.workouts) {
      const key = startOfDay(w.date);
  ...
  // Beregn alle datoer i kalender-rutenettet (6 uker × 7 dager)
  const cells = useMemo(() => {
    const firstOfMonth = new Date(view.year, view.month, 1);
    const dayOfWeek = firstOfMonth.getDay(); // 0=søndag
  ...
  const monthsThisYear = workoutsByDay;
  const workoutsInView = useMemo(() => {
    return data.workouts.filter(w => {
      const d = new Date(w.date);
      return d.getFullYear() === view.year && d.getMonth() === view.month;
    });
  }, [data.workouts, view.year, view.month]);

  const totalVolInView = useMemo(() =>
    workoutsInView.reduce((sum, w) => sum + totalVolume(w), 0)
  , [workoutsInView]);
  ...
  const onDayClick = (d) => {
    ...
    const ws = workoutsByDay.get(key);
    if (!ws || ws.length === 0) return;
    // Hvis flere økter: ta den siste
    const w = ws[ws.length - 1];
    navigate('historikk-detalj', { workoutId: w.id });
  };
  ...
  return html`
    <div class="screen-enter pb-24">
      <${Header}
        title="Kalender"
        subtitle="Treningshistorikk"
        right=${html`
          <button onClick=${() => navigate('hjem')} class="tap inline-flex items-center justify-center w-10 h-10 rounded-full text-ink-900 active:bg-ink-100" aria-label="Tilbake">
      ...
      <div class="px-5 mb-4">
        <${Card} className="p-3 flex items-center justify-between">
          <button onClick=${goPrev} class="tap w-9 h-9 rounded-lg flex items-center justify-center text-ink-700 active:bg-ink-200" aria-label="Forrige måned">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          </button>
          <div class="flex flex-col items-center">
            <div class="text-base font-semibold">${monthLabelCap}</div>
            ${!isCurrentMonth && html`
              <button onClick=${goToday} class="tap text-[11px] uppercase tracking-wide text-accent active:text-accent-hover mt-0.5">Til i dag</button>
            `}
          </div>
          <button onClick=${goNext} class="tap w-9 h-9 rounded-lg flex items-center justify-center text-ink-700 active:bg-ink-200" aria-label="Neste måned">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
          </button>
        <//>
      </div>

      <!-- Statistikk for valgt måned -->
      <div class="px-5 mb-4 grid grid-cols-2 gap-2">
        <${Card} className="p-3">
          <div class="text-[10px] uppercase tracking-wide text-ink-400 font-medium">Økter denne måneden</div>
          <div class="text-2xl font-semibold big-num">${workoutsInView.length}</div>
        <//>
        <${Card} className="p-3">
          <div class="text-[10px] uppercase tracking-wide text-ink-400 font-medium">Totalt volum</div>
          <div class="text-2xl font-semibold big-num">${Math.round(totalVolInView).toLocaleString('no-NO')} <span class="text-sm text-ink-400 font-normal">${data.settings.units}</span></div>
        <//>
      </div>
      ...
      <div class="px-5">
        <${Card} className="p-3">
          ...
          <div class="grid grid-cols-7 mb-2">
            ${['M','T','O','T','F','L','S'].map(d => html`
              <div class="text-[10px] uppercase tracking-wide text-ink-400 text-center">${d}</div>
            `)}
          </div>
          <!-- Dag-rutenett -->
          <div class="grid grid-cols-7 gap-1">
            ${cells.map(d => {
              ...
              const ws = workoutsByDay.get(key);
              const hasWorkout = !!ws && ws.length > 0;
              ...
              return html`
                <button
                  ...
                  disabled=${!hasWorkout}
                  class=${cx(
                    'aspect-square flex flex-col items-center justify-center rounded-lg text-sm relative tap',
                    !inMonth && 'opacity-30',
                    isToday && !hasWorkout && 'ring-1 ring-ink-300',
                    hasWorkout && 'bg-accent text-accent-fg font-semibold active:bg-accent-hover',
                    !hasWorkout && inMonth && !isFuture && 'text-ink-700',
                    !hasWorkout && (isFuture || !inMonth) && 'text-ink-500',
                    !hasWorkout && 'cursor-default',
                  )}
                >
                  <span>${d.getDate()}</span>
                  ${isToday && hasWorkout && html`<span class="absolute bottom-1 w-1 h-1 rounded-full bg-accent-fg"></span>`}
                  ${ws && ws.length > 1 && html`<span class="absolute top-1 right-1 text-[8px] font-semibold">${ws.length}</span>`}
                </button>
              `;
            })}
          </div>
          <!-- Forklaring -->
          <div class="flex items-center gap-3 mt-3 pt-3 border-t border-ink-200">
            <div class="flex items-center gap-1.5">
              <span class="w-3 h-3 rounded-md bg-accent"></span>
              <span class="text-xs text-ink-500">Trent</span>
            </div>
            <div class="flex items-center gap-1.5">
              <span class="w-3 h-3 rounded-md ring-1 ring-ink-300"></span>
              <span class="text-xs text-ink-500">I dag</span>
            </div>
          </div>
        <//>
      </div>

      ${workoutsInView.length === 0 && html`
        <div class="px-5 mt-4">
          <div class="text-sm text-ink-400 text-center py-4">Ingen økter denne måneden ennå.</div>
        </div>
      `}
    </div>
  `;
...
function App() {
  ...
  const renderRoute = () => {
    switch (route.name) {
      ...
      case 'kalender': return html`<${CalendarScreen} data=${data} navigate=${navigate} />`;
      case 'historikk':
